...
NOTE: Trident non supporta i pool di capacità QoS manuali.
...
`exportRule` Deve essere un elenco separato da virgole di qualsiasi combinazione di indirizzi IPv4 o sottoreti IPv4 nella notazione CIDR. Ignorato per i volumi SMB. | "0.0.0.0/0" 
...
NOTE: `nasType: smb` Filtri per i pool che supportano volumi SMB. `nasType: nfs` O `nasType: null` filtri per pool NFS.
